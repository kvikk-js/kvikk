...
// On Windows a file URL object will start with something like "/D:/"
// and when converting file paths to URL paths this causes URL paths
// on Windows to contain this as an unwanted section in the pathname.
// This creates an root path we can use to subtract this unwanted section
// from other URL paths.
const pathRoot = pathToFileURL('/').pathname;

/*
// @ts-ignore
const pathOrg = path.join(import.meta.dirname, '../../package.json');
const pathToFile = pathToFileURL(pathOrg);
const pathTURL = fileURLToPath(pathToFile);
console.log('TEST pathOrg', pathOrg);
console.log('TEST pathToFile', pathToFile);
console.log('TEST pathTURL', pathTURL);
*/
...
export default class Config {
   ...
   */
  get pkgDefaults() {
    return pathToFileURL(this.#pkgDefaults);
  }
   ...
   */
  get dirBuild() {
    // const joined = path.join(this.dirSrc, this.#dirBuild);
   ...
   */
  get dirComponents() {
    // const joined = path.join(this.dirSrc, this.#dirComponents);
   ...
   */
  get dirLayouts() {
    // const joined = path.join(this.dirSrc, this.#dirLayouts);
   ...
   */
  get dirPublic() {
    // const joined = path.join(this.dirSrc, this.#dirPublic);
   ...
   */
  get dirSystem() {
    // const joined = path.join(this.dirSrc, this.#dirSystem);
   ...
   */
  get dirPages() {
    // const joined = path.join(this.dirSrc, this.#dirPages);
  ...
  urlPathToJs(appendix = '') {
    ...
    const base = pathToFileURL(path.join(this.#urlPathBase, env, this.#dirJs, '/', appendix));
    return new URL(base.pathname.replace(pathRoot, '/'), url);
  }
}
