...
export default class Config {
   ...
   * has sane defaults.
   *
   * The user config (kvikk.config.js) is merged into this config.
   *
  ...
  async load(configuration) {
    ...
    if (configuration) {
      ...
      try {
        const { default: conf } = await import(path.join(this.#cwd, 'kvikk.config.js'));
        userConfig = conf;
  ...
  /**
   * The version of the Kvikk.js module in use.
   *
   ...
   * the kvikk.js command in. This defaults to, and has the same
   ...
   * all routes in the application and will prefix all URL
   * in the application.
   *
   * Example:
   * http://localhost:4000/<pathbase>/<routes>...
   *
   * Defaults to /
   *
   * @type {string}
