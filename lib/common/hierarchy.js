...
export default class HierarchyRoutes {
  ...
  async setDocument() {
    ...
    const filePath = new URL('./document.js', this.config.dirSystem);
    try {
      const stats = await fs.stat(filePath);
      if (stats.isFile()) {
        this.document = filePath;
        this.logger.trace(`Using user defined document.js at ${fileURLToPath(this.document)}`);
      }
    } catch (err) { // eslint-disable-line
      // this.document = join(this.config.pkgDefaults, '/system', 'document.js');
      this.document = new URL('./system/document.js', this.config.dirSystem);
  ...
  async setNotFound() {
    ...
    const filePath = new URL('./not-found.js', this.config.dirSystem);
    try {
      const stats = await fs.stat(filePath);
      if (stats.isFile()) {
        this.notFound = filePath;
        this.logger.trace(`Using user defined not-found.js at ${fileURLToPath(this.notFound)}`);
      }
    } catch (err) { // eslint-disable-line
      // this.notFound = join(this.config.pkgDefaults, '/system', 'not-found.js');
      this.notFound = new URL('./system/not-found.js', this.config.dirSystem);
  ...
  async setError() {
    ...
    const filePath = new URL('./error.js', this.config.dirSystem);
    try {
      const stats = await fs.stat(filePath);
      if (stats.isFile()) {
        this.error = filePath;
        this.logger.trace(`Using user defined error.js at ${fileURLToPath(this.error)}`);
      }
    } catch (err) { // eslint-disable-line
      // this.error = join(this.config.pkgDefaults, '/system', 'error.js');
      this.error = new URL('./system/error.js', this.config.dirSystem);
