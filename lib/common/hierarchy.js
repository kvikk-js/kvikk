...
export default class HierarchyRoutes {
  ...
  async setRoutes() {
    ...
    for await (const entry of this.routesGlob) {
      ...
      console.log('RAW', entry.parentPath);
      console.log('PARENT_PATH', uriParentPath);
      console.log('FULL_PATH', uriFullPath);
      console.log('DIRPAGES', this.config.dirPages);

      console.log('XXX', uriParentPath.pathname.replace(this.config.dirPages.pathname, '/'));
  ...
  setAction(parentPath, source) {
    ...
    route = new URL(`.${route}`, this.config.urlPathBase).pathname;
  ...
  setPage(parentPath, source) {
    let route = parentPath.pathname.replace(this.config.dirPages.pathname, '/');
    // if (route === '') route = '/'; // Make sure root route is a /
    // route = join(this.config.urlPathBase.pathname, route); // Append base path if configured
    route = new URL(`.${route}`, this.config.urlPathBase).pathname;
