        ...
        try {
          ...
          fastify.get(
            ...
            async (request, reply) => {
              ...
              const scriptPrefix = config.development ? '_' : 'public';

              const scriptHydrationUrl = join(config.urlBasePath, scriptPrefix, '/lit/hydration.js');
              request.header.setScript(scriptHydrationUrl);

              const scriptPageFile = `${request.routeOptions.config.entry.hash}.js`;
              const scriptPageUrl = join(config.urlBasePath, scriptPrefix, `pages`, scriptPageFile);
